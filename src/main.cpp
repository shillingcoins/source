...
const string strMessageMagic = "Shilling Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uSHI
...
void ThreadScriptCheck() {
    RenameThread("shilling-scriptch");
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ShillingMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ShillingMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ShillingMiner(CWallet *pwallet)
{
    printf("ShillingMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("shilling-miner");
    ...
    try { loop {
        ...
        printf("Running ShillingMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ShillingMiner terminated\n");
        throw;
    }
}

void GenerateShillings(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ShillingMiner, pwallet));
}
