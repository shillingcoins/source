...
{
    QString ret = QString("shilling:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SHI unit input in lnReqAmount, we generate the URI with SHI as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(ShillingUnits::format(ShillingUnits::SHI, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
